...
int main(int argc, char** argv) {
  std::shared_ptr<nv3dvc::applications::LocalVideoConferencingApplication> app;
  try {
    app = std::make_shared<nv3dvc::applications::LocalVideoConferencingApplication>();
  } catch (std::exception& e) {
    std::cerr << "Failed to create application: " << e.what() << std::endl;
    return 1;
  }
  std::string app_config_path = "LocalVideoConferencingApplication.json";
